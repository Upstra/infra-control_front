<template>
  <div>
    ...
    <transition name="fade">
      <div
        v-if="isOpen"
        class="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4"
      >
        <div class="bg-white w-full max-w-xl rounded-xl shadow-xl p-4 mt-20">
          <div class="flex items-center justify-between">
            ...
            />
            <span
              class="ml-4 px-2 py-1 bg-neutral-100 rounded text-sm text-neutral-500 cursor-pointer"
          ...
          <ul class="mt-4 space-y-2 max-h-96 overflow-y-auto">
            <template v-for="(grouped, group) in groupedActions" :key="group">
              <li class="text-xs text-neutral-400 uppercase px-3 mt-2">
                {{ group }}
              </li>
              <li
                v-for="action in grouped"
                :key="action.label"
...
const actions = [
  {
    group: 'command_palette.groups.users',
    label: 'command_palette.show_users',
    icon: Users,
    fn: () => navigateTo('/users'),
    shortcut: 'U',
    path: '/users',
  ...
  },
  {
    ...
    icon: Group,
    fn: () => emit('switchView', 'card'),
    onlyInUsers: true,
  },
  {
    group: 'command_palette.groups.servers',
  ...
  {
    group: 'command_palette.groups.infrastructure',
  ...
  {
    group: 'command_palette.groups.servers',
    label: 'command_palette.view_logs',
    icon: Server,
    fn: () => navigateTo('/logs'),
    path: '/logs',
  },
  {
    ...
    label: 'command_palette.view_settings',
    icon: Cog6ToothIcon,
  ...
  {
    group: 'command_palette.groups.general',
    label: 'command_palette.toggle_dark',
    icon: Plug,
    fn: () => toggleDarkMode(),
    shortcut: 'T',
  },
