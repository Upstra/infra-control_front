            ...
            <DialogPanel class="w-full max-w-4xl transform overflow-hidden rounded-2xl bg-white dark:bg-gray-900 shadow-xl transition-all">
              <form @submit.prevent="handleSubmit">
                ...
                <div class="px-6 py-4 space-y-6">
                  ...
                  <div v-if="formData.type === 'vm' && !isEditing" class="space-y-2">
                    ...
                    <select
                      ...
                      class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    >
                      <option value="">{{ $t('groups.selectServerGroup') }}</option>
                      <option v-for="group in serverGroups" :key="group.id" :value="group.id">
                        {{ group.name }}
                      </option>
                    </select>
                  </div>
                  ...
                  <div>
                    <div class="flex items-center justify-between mb-4">
                      <h3 class="text-lg font-medium text-gray-900 dark:text-white">
                        {{ formData.type === 'server' ? $t('groups.servers') : $t('groups.vms') }}
                      </h3>
                      <span class="text-sm text-gray-500 dark:text-gray-400">
                        {{ selectedResourceIds.length }} {{ $t('groups.selected') }}
                      </span>
                    </div>
...
import type { GroupServerResponseDto, GroupVmResponseDto, CreateGroupPayload, UpdateGroupPayload } from '../types';
...
const availableResources = ref<Array<{ id: string; name: string; state?: string; roomId?: string }>>([]);
...
watch(() => props.group, (group) => {
  if (group) {
    ...
    if (group.type === 'server') {
      selectedResourceIds.value = (group as GroupServerResponseDto).serverIds || [];
    } else {
