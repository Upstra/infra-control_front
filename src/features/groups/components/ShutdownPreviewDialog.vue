<template>
  ...
  >
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="relative inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full dark:bg-gray-800">
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 dark:bg-gray-800">
          <div class="sm:flex sm:items-start">
            <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-yellow-100 sm:mx-0 sm:h-10 sm:w-10 dark:bg-yellow-900/20">
              <ExclamationTriangleIcon class="h-6 w-6 text-yellow-600 dark:text-yellow-400" aria-hidden="true" />
            </div>
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                ...
                <div class="flex items-center justify-between mb-4">
                  <div class="flex items-center gap-4">
                    <div class="text-sm text-gray-600 dark:text-gray-300">
                      <span class="font-medium">{{ groupStore.shutdownPreview.totalVms }}</span> VMs,
                      <span class="font-medium">{{ groupStore.shutdownPreview.totalServers }}</span> Servers
                    </div>
                    <div class="text-sm text-gray-600 dark:text-gray-300">
                      {{ $t('pagination.showing') }} 
                      {{ ((groupStore.shutdownPreview.currentPage - 1) * 10) + 1 }}-{{ Math.min(groupStore.shutdownPreview.currentPage * 10, groupStore.shutdownPreview.totalItems) }}
                      {{ $t('pagination.of') }} {{ groupStore.shutdownPreview.totalItems }}
                    </div>
                  </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-600">
                      ...
                      >
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                          <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 text-blue-800 text-sm font-semibold dark:bg-blue-900/20 dark:text-blue-400">
                            {{ step.order }}
                          </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                          <div class="flex items-center gap-2">
                            <CpuChipIcon v-if="step.type === 'vm'" class="h-5 w-5 text-blue-600 dark:text-blue-400" />
                            <ServerIcon v-else class="h-5 w-5 text-green-600 dark:text-green-400" />
              ...
              >
                ...
                <span class="ml-3 text-gray-600 dark:text-gray-300">{{ $t('groups.loadingPreview') }}</span>
              </div>
              ...
              <div
                v-if="groupStore.shutdownPreview && !groupStore.isLoadingPreview"
              ...
              >
                <div class="flex items-start gap-2">
                  ...
                  <div class="text-sm text-yellow-800 dark:text-yellow-300">
                    <p class="font-medium">{{ $t('groups.confirmationRequired') }}</p>
                    <p class="mt-1">{{ $t('groups.shutdownConfirmationText') }}</p>
                  </div>
                </div>
              </div>
        ...
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse dark:bg-gray-700">
          ...
          >
            <div v-if="groupStore.isExecutingShutdown" class="flex items-center gap-2">
...
import { ExclamationTriangleIcon, CpuChipIcon, ServerIcon } from '@heroicons/vue/24/outline';
