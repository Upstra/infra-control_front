  ...
  <div v-if="viewMode !== 'list'" :class="cardClasses">
    ...
    <div class="grid grid-cols-2 gap-4 mb-6">
      <div class="text-center p-3 bg-slate-50 rounded-xl">
        <ServerIcon class="h-6 w-6 text-slate-600 mx-auto mb-1" />
        <div class="text-lg font-bold text-slate-900">{{ stats.servers }}</div>
        <div class="text-xs text-slate-600">{{ t('servers.servers') }}</div>
      </div>
      <div class="text-center p-3 bg-slate-50 rounded-xl">
    ...
    <div class="flex items-center space-x-4 flex-1">
      ...
      <div class="hidden sm:flex items-center space-x-6 text-sm">
        <div class="text-center">
          ...
          <div class="text-slate-600">{{ t('servers.servers') }}</div>
        </div>
        <div class="text-center">
          <div class="font-semibold text-slate-900">{{ stats.ups }}</div>
          <div class="text-slate-600">{{ t('nav.ups') }}</div>
        </div>
        <div class="text-center">
          <div class="font-semibold text-green-600">{{ stats.uptime }}%</div>
