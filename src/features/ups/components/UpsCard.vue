...
const getLoadColor = (load: number) => {
  ...
  if (load >= 60) return 'text-amber-600 bg-amber-100';
...
<template>
  <div
    class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 space-y-4 hover:shadow-md transition-all duration-200 cursor-pointer"
  >
    <!-- Header -->
    <div class="flex items-center justify-between">
      <div class="flex items-center space-x-3">
        ...
        <div>
          <h3 class="text-lg font-semibold text-slate-900">{{ ups.name }}</h3>
          <p class="text-sm text-slate-600">{{ ups.ip }}</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4 text-sm">
        <div class="space-y-1">
          <p class="text-slate-600">
            ...
            <span class="text-slate-900 ml-1">{{ ups.grace_period_on }}s</span>
          </p>
          <p class="text-slate-600">
            <span class="font-medium">{{ t('ups.grace_off') }}:</span>
            <span class="text-slate-900 ml-1">{{ ups.grace_period_off }}s</span>
          </p>
        </div>
        ...
        <div class="flex items-center justify-end">
          <div class="text-center">
            <div class="flex items-center justify-center mb-1">
              <ServerIcon class="h-4 w-4 text-blue-600" />
            </div>
    ...
    >
      <div class="flex items-center space-x-2">
        <ExclamationTriangleIcon class="h-4 w-4 text-red-600 flex-shrink-0" />
        <div class="text-sm">
          <p v-if="upsMetrics.status === 'offline'" class="text-red-800">
            {{ t('ups.warning_offline') }}
          </p>
          <p v-else-if="upsMetrics.load >= 80" class="text-red-800">
            {{ t('ups.warning_high_load') }}
          </p>
          <p v-else-if="upsMetrics.batteryLevel <= 20" class="text-red-800">
