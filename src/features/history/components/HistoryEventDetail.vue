      ...
      case 'LOGIN_SUCCESS':
        return {
          ...
          description: t('history.descriptions.login_success', { 
            ...
            email: metadata?.userEmail || props.event.user?.email || props.event.userId 
          })
        };
      case 'LOGIN_FAILED':
        return {
      ...
      case 'CREATE':
        return {
          ...
          description: t('history.descriptions.vm_created', {
            ...
            server: props.event.newValue?.serverHostname || metadata?.parentServer
          })
        };
    }
  }
        ...
        if (opType === 'ADD_ROLE') {
          return {
            ...
            description: t('history.descriptions.role_added', {
              ...
              admin: metadata?.targetRoleIsAdmin ? t('common.yes') : t('common.no')
            })
          };
...
const formatValue = (value: any): string => {
  ...
  if (typeof value === 'object') {
    if (Array.isArray(value)) {
      return value.map(v => typeof v === 'object' ? v.name || JSON.stringify(v) : v).join(', ');
    }
...
const getContextualDetails = computed(() => {
  ...
  // IP Address (show first if available)
  if (props.event.ipAddress) {
    details.push({ label: t('history.details.ip_address'), value: props.event.ipAddress });
  }
  
  if (metadata) {
    if (entity === 'auth') {
      if (metadata.loginMethod) {
        details.push({ label: t('history.details.login_method'), value: metadata.loginMethod });
      }
      if (metadata.requires2FA !== undefined) {
        details.push({ label: t('history.details.requires_2fa'), value: metadata.requires2FA });
      }
    }
    
    if (entity === 'server' || entity === 'vm') {
      if (metadata.serverType) {
        details.push({ label: t('history.details.server_type'), value: metadata.serverType });
      }
      if (metadata.operatingSystem) {
        details.push({ label: t('history.details.os'), value: metadata.operatingSystem });
      }
      if (metadata.assignedToGroup !== undefined) {
        details.push({ label: t('history.details.assigned_to_group'), value: metadata.assignedToGroup });
      }
    }
    ...
    if (entity === 'user_role') {
      ...
      if (metadata.isElevationToAdmin) {
        details.push({ label: t('history.details.admin_elevation'), value: true });
      }
      if (metadata.isRemovalFromAdmin) {
        details.push({ label: t('history.details.admin_removal'), value: true });
      }
    }
  }
...
<template>
  <div class="space-y-4 p-6 bg-white dark:bg-neutral-800 rounded-lg shadow-lg max-w-5xl">
    <!-- Header with action info -->
    <div class="flex items-start gap-4 pb-4 border-b border-gray-200 dark:border-neutral-700">
      <div class="flex-shrink-0 p-3 rounded-full bg-gray-100 dark:bg-neutral-700">
      ...
      <div class="flex-1">
        ...
        <div class="mt-2 flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
          ...
          <span v-if="event.userId || event.user" class="flex items-center gap-1">
      ...
      <span :class="actionStyle.color" class="px-3 py-1 text-sm font-medium rounded-full bg-gray-100 dark:bg-neutral-700">
        {{ event.action }}
      </span>
    </div>

    <!-- User Information Section -->
    <div v-if="event.user" class="bg-gray-50 dark:bg-neutral-900 rounded-lg p-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div class="flex justify-between">
          <span class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ t('history.details.user_email') }}:</span>
          <span class="text-sm text-gray-900 dark:text-white">{{ event.user.email }}</span>
        </div>
        <div v-if="event.user.username" class="flex justify-between">
          <span class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ t('history.details.username') }}:</span>
          <span class="text-sm text-gray-900 dark:text-white">{{ event.user.username }}</span>
        </div>
        <div v-if="event.user.firstName || event.user.lastName" class="flex justify-between">
          <span class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ t('history.details.full_name') }}:</span>
          <span class="text-sm text-gray-900 dark:text-white">
            {{ event.user.firstName }} {{ event.user.lastName }}
          </span>
        </div>
        <div v-if="event.user.roles && event.user.roles.length > 0" class="flex justify-between">
          <span class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ t('history.details.user_roles') }}:</span>
          <span class="text-sm text-gray-900 dark:text-white">
            <span v-for="(role, idx) in event.user.roles" :key="role.id">
              {{ role.name }}<span v-if="role.isAdmin" class="text-yellow-600 dark:text-yellow-400"> ★</span><span v-if="idx < event.user.roles.length - 1">, </span>
            </span>
          </span>
        </div>
        <div class="flex justify-between">
          <span class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ t('history.details.user_active') }}:</span>
          <span class="text-sm">
            <span v-if="event.user.active" class="text-green-600 dark:text-green-400">✓ {{ t('common.yes') }}</span>
            <span v-else class="text-red-600 dark:text-red-400">✗ {{ t('common.no') }}</span>
          </span>
        </div>
      </div>
    </div>
    ...
    <!-- Contextual Details -->
    <div v-if="getContextualDetails.length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div v-for="detail in getContextualDetails" :key="detail.label" 
           class="flex justify-between p-3 bg-gray-50 dark:bg-neutral-900 rounded-lg">
        <span class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ detail.label }}:</span>
        <span class="text-sm text-gray-900 dark:text-white font-medium">
          <span v-if="typeof detail.value === 'boolean'">
            <span v-if="detail.value" class="text-green-600">✓ {{ t('common.yes') }}</span>
    ...
    <div v-if="hasChanges" class="space-y-3">
      ...
      <div class="space-y-3">
        <div v-for="change in getChangedFields" :key="change.field" 
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 divide-x divide-gray-200 dark:divide-neutral-700">
            <div class="p-4">
              <p class="text-xs font-medium text-red-600 dark:text-red-400 mb-2">
                {{ t('administration.history_details.old_value') }}
              </p>
              <div class="text-sm text-gray-700 dark:text-gray-300">
                <pre v-if="typeof change.old === 'object' && change.old !== null" 
                     class="text-xs bg-gray-100 dark:bg-neutral-800 p-2 rounded overflow-x-auto">{{ formatJson(change.old) }}</pre>
            ...
            <div class="p-4">
              ...
              <div class="text-sm text-gray-700 dark:text-gray-300">
                <pre v-if="typeof change.new === 'object' && change.new !== null" 
                     class="text-xs bg-gray-100 dark:bg-neutral-800 p-2 rounded overflow-x-auto">{{ formatJson(change.new) }}</pre>
    ...
    <!-- Additional Metadata -->
    <div v-if="event.metadata && Object.keys(event.metadata).length > 0" class="space-y-3">
      <h4 class="font-medium text-gray-700 dark:text-gray-300 flex items-center gap-2">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="bg-gray-50 dark:bg-neutral-900 rounded-lg p-4">
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <template v-for="(value, key) in event.metadata" :key="String(key)">
            <div v-if="!['userEmail', 'identifier', 'loginMethod', 'requires2FA', 'operationType', 'targetRoleName', 'targetRoleIsAdmin', 'previousRoleCount', 'newRoleCount', 'isElevationToAdmin', 'isRemovalFromAdmin', 'serverType', 'operatingSystem', 'assignedToGroup'].includes(String(key))"
    ...
    <!-- Technical Info -->
    <div v-if="event.userAgent || event.correlationId" 
      ...
      <div class="space-y-1 text-xs text-gray-500 dark:text-gray-400 font-mono">
        ...
        <p v-if="event.correlationId">
          <span class="font-medium">Correlation ID:</span> {{ event.correlationId }}
        </p>
      </div>
    </div>
  </div>
