...
<template>
  ...
  >
    <div class="flex items-center justify-between">
      <h3 class="text-lg font-bold text-slate-900 dark:text-white">{{ release.name }}</h3>
      <span class="text-xs text-neutral-dark dark:text-neutral-400 opacity-70 whitespace-nowrap">
        {{ new Date(release.publishedAt).toLocaleDateString() }}
        <template v-if="release.author"> — {{ release.author }}</template>
      </span>
    </div>
    <div v-html="rendered" class="prose prose-sm dark:prose-invert max-w-none text-sm mt-1"></div>
    <a
