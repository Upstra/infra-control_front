...
const handleSubmit = async () => {
    ...
    try {
        ...
        errorMsg.value =
            e?.response?.data?.message?.[0] ?? e?.message ?? t('profile.unexpected_error')
    } finally {
        loading.value = false
    }
}
        ...
        <div v-if="props.open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div class="relative w-full max-w-md rounded-xl bg-white p-6 shadow-xl" @keydown.esc="emit('close')">
                ...
                <div class="mb-6 flex flex-col items-center gap-2">
                    ...
                    <h3 class="text-xl font-semibold text-neutral-darker">{{ t('profile.reset_password_title') }}</h3>
                </div>
                <form class="space-y-4" @submit.prevent="handleSubmit">
                    <div class="space-y-1">
                        <label class="text-sm font-medium text-neutral-darker">{{ t('profile.new_password') }}
                    ...
                    <div class="space-y-1">
                        <label class="text-sm font-medium text-neutral-darker">{{ t('profile.confirm') }}
