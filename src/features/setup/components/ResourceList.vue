<template>
  <div class="resource-list">
    ...
    <div v-if="items.length === 0" class="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-lg">
      ...
      >
        ...
        <div v-if="showStatus && item.status" class="mt-3">
          <span 
            ...
            :class="{
              'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200': item.status === 'active' || item.status === 'connected',
              'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200': item.status === 'pending',
              'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200': item.status === 'error' || item.status === 'disconnected',
              'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200': !['active', 'connected', 'pending', 'error', 'disconnected'].includes(item.status)
            }"
          >
            {{ item.status }}
          </span>
    ...
    <div v-if="showBulkActions && items.length > 0" class="mt-6 flex justify-between items-center">
