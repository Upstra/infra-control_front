        ...
        <form @submit.prevent="handleSubmit"
            ...
            <div>
                ...
                <input id="name" v-model="form.name" type="text"
                    class="block w-full border border-neutral-300 rounded-lg px-3 py-2 text-base focus:ring-2 focus:ring-primary focus:border-primary transition"
                    :placeholder="t('setup_ups.name_placeholder')" required maxlength="64" />
                <span class="text-xs text-neutral mt-1 block">{{ t('setup_ups.name_hint') }}</span>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input id="ip" v-model="form.ip" type="text"
                        class="block w-full border border-neutral-300 rounded-lg px-3 py-2 text-base focus:ring-2 focus:ring-primary focus:border-primary transition"
                        :placeholder="t('setup_ups.ip_placeholder')" :pattern="ipv4Pattern" required />
                    <span class="text-xs text-neutral mt-1 block">{{ t('setup_ups.ip_hint') }}</span>
                </div>
                <div>
                    ...
                    <input id="login" v-model="form.login" type="text"
                        class="block w-full border border-neutral-300 rounded-lg px-3 py-2 text-base focus:ring-2 focus:ring-primary focus:border-primary transition"
                        :placeholder="t('setup_ups.login_placeholder')" required />
                </div>
            </div>
            ...
            <div>
                ...
                <input id="password" v-model="form.password" type="password"
                    class="block w-full border border-neutral-300 rounded-lg px-3 py-2 text-base focus:ring-2 focus:ring-primary focus:border-primary transition"
                    :placeholder="t('setup_ups.password_placeholder')" required />
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input id="grace_period_on" v-model.number="form.grace_period_on" type="number" min="0"
                        class="block w-full border border-neutral-300 rounded-lg px-3 py-2 text-base focus:ring-2 focus:ring-primary focus:border-primary transition"
                        :placeholder="t('setup_ups.grace_on_placeholder')" required />
                    <span class="text-xs text-neutral mt-1 block">{{ t('setup_ups.grace_on_hint') }}</span>
                </div>
                <div>
                    ...
                    <input id="grace_period_off" v-model.number="form.grace_period_off" type="number" min="0"
                        class="block w-full border border-neutral-300 rounded-lg px-3 py-2 text-base focus:ring-2 focus:ring-primary focus:border-primary transition"
                        :placeholder="t('setup_ups.grace_off_placeholder')" required />
                    <span class="text-xs text-neutral mt-1 block">{{ t('setup_ups.grace_off_hint') }}</span>
                </div>
            </div>
            ...
            <div>
                ...
                <input id="capacity" v-model.number="form.capacity" type="number"
                    class="block w-full border border-neutral-300 rounded-lg px-3 py-2 text-base focus:ring-2 focus:ring-primary focus:border-primary transition"
                    min="1" step="0.1" :placeholder="t('setup_ups.capacity_placeholder')" />
                <span class="text-xs text-neutral mt-1 block">{{ t('setup_ups.capacity_hint') }}</span>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                <div class="flex items-center gap-3 bg-primary/5 rounded-lg p-4">
                    ...
                    <div>
                        <span class="font-semibold text-neutral-dark">{{ t('setup_ups.runtime_title') }}</span>
                        <p class="text-xs text-neutral mt-1">{{ t('setup_ups.runtime_text', { minutes: estimatedRuntime }) }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-3 bg-primary/5 rounded-lg p-4">
                    <Server :size="22" class="text-primary" />
                    <div>
                        <span class="font-semibold text-neutral-dark">{{ t('setup_ups.server_capacity_title') }}</span>
                        <p class="text-xs text-neutral mt-1">{{ t('setup_ups.server_capacity_text', { count: estimatedServerCapacity }) }}
                        </p>
                    </div>
                </div>
            </div>
...
const handleSubmit = async () => {
    ...
    if (!ipv4Regex.test(form.ip ?? '')) return toast.error(t('setup_ups.ip_invalid'));
    ...
    if (form.grace_period_on < 0 || form.grace_period_off < 0) return toast.error(t('setup_ups.negative_delay_error'));
    ...
    try {
        ...
        const errorMessage = err.response?.data?.message || err.message || t('setup_ups.error');
