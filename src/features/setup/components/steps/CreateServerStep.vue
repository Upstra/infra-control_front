...
const testServerPing = async (ip: string) => {
  if (!savedServerId) {
    const payload = {
      ...
      ip: ip.trim(),
      state: 'UP' as ServerState,
      adminUrl: 'http://temp',
      login: 'temp',
      password: 'temp',
      type: 'physical' as ServerType,
      priority: 1,
      grace_period_on: 60,
      grace_period_off: 30,
      roomId: form.roomId,
    ...
    // Si l'iLO a été créé par l'API, on met à jour le formulaire avec les données retournées
    if (savedServer.ilo) {
      ...
      // On ne met pas à jour le password pour des raisons de sécurité
    }
  } else {
